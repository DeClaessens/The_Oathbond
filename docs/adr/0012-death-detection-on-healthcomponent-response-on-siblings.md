...
- `Events.character_died` carries the fatal hit's `source` as `killer`, which may be `null` (environmental damage, tests). The XP system must decide what a null killer means before awarding anything.
